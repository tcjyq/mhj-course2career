...
用户名/密码验证后，服务端生成 7 天随机 opaque token，只在 `auth_sessions` 保存 SHA-256 哈希；浏览器 cookie 不包含密码、用户名、角色、套餐或 Key。刷新和重新打开标签页应恢复登录；登出、停用账号、删除用户、密码或 `session_version` 变化后旧会话不得恢复。角色、套餐与开发者模式从数据库实时读取。生产仍只用 PostgreSQL，连接失败安全停止。
