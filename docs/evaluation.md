...
使用合成账号核对 SQLite/PostgreSQL 会话创建、哈希、过期、撤销、用户隔离、停用、密码轮换、管理员与开发者状态；真实本地浏览器核对 F5、Ctrl+R、新标签页、登出后刷新和 390px。浏览器只检查 cookie 是否存在，不输出 token 值；URL 不含 token。真实 Provider API 不参与此回归。
