...
管理员入口只对`admin + Admin`显示。“我的 AI Provider”只对具备开发者 Key 权限的 Developer/Admin 显示；游客与普通用户的导航和分析模式均不出现 BYOK。页面可见性只改善体验，最终权限仍由服务层验证。
...
- C08-B1 开发者页展示十家受控官方预设，分析页仅在 Developer/Admin 有已保存 Key 和模型 ID 时显示“开发者API Key”，并只列该账户已配置的 Provider。未验证模型显示明确提示；免费套餐系统AI只显示 DeepSeek。预设可配置不等于真实模型验证通过；不提供任意自定义端点输入。
