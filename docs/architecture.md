...
项目使用单体Streamlit与`src`布局。`app.py`负责产品外壳、会话身份和动态导航，`ui/`页面负责输入与展示；领域模块负责校验、映射、评分和导出；权限与仓储层负责用户、额度和历史。当前使用SQLite，仓储边界允许后续迁移PostgreSQL。
...
| `scoring.py` | 单项状态和岗位综合分 |
| `learning_path.py` | 根据差距生成确定性学习步骤 |
| `analysis_service.py` | 串联领域流程并形成报告模型 |
...
公开部署前需要增加网关级游客限流、登录防爆破、Token/成本结算和管理员角色授予流程。用户量增长后将SQLite仓储替换为PostgreSQL，并考虑增加独立API服务。
